...
public class MobEffectEpilepsyFireBurning extends Potion {
    private final static String ICON_NAME_0 = Reference.MOD_ID + ":blocks/yellow_flame_layer_0";
    private final static String ICON_NAME_1 = Reference.MOD_ID + ":blocks/yellow_flame_layer_1";

    public MobEffectEpilepsyFireBurning(boolean isBadEffectIn, int liquidColorIn) {
        super(isBadEffectIn, liquidColorIn);
        PotionUtil.registerPotion(this, "epilepsy_fire_burning");
        KaliaStylePotion.POTIONS.add(this);
    }

    public static Potion getPotion() {
        return KaliaStylePotion.EPILEPSY_FIRE_BURNING;
    }
        ...
        if (evt.getPotion().equals(getPotion())) {
            ...
            if (minecraftServer != null) {
                for (EntityPlayer entityPlayer : entityLivingBase.world.getMinecraftServer().getPlayerList().getPlayers()) {
                    NetworkRegistryHandler.EpilepsyFireBurning.sendClientCustomPacket(entityPlayer, entityLivingBase.getEntityId(), false);
                }
            }
        }
    }
    ...
    public static void onRenderLiving(RenderLivingEvent.Specials.Post evt) {
        ...
        if (NetworkRegistryHandler.EpilepsyFireBurning.getEntitiesID().contains(entityLivingBase.getEntityId())) {
    ...
    public static void onRenderSpecificHand(RenderSpecificHandEvent evt) {
        ...
        if (NetworkRegistryHandler.EpilepsyFireBurning.getEntitiesID().contains(player.getEntityId())) {
    ...
    public void performEffect(EntityLivingBase entityLivingBaseIn, int amplifier) {
        ...
        if (minecraftServer != null) {
            for (EntityPlayer entityPlayer : entityLivingBaseIn.world.getMinecraftServer().getPlayerList().getPlayers()) {
                NetworkRegistryHandler.EpilepsyFireBurning.sendClientCustomPacket(entityPlayer, entityLivingBaseIn.getEntityId(), true);
            }
        }
    }

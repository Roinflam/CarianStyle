...
public class EntityGlintblades extends EntityThrowable {
    ...
    public EntityGlintblades(World worldIn) {
        super(worldIn);
        setTarget(null);
    ...
    public EntityGlintblades(EntityLivingBase throwerIn, Entity target) {
    ...
    public void shoot(float velocity) {
        if (target != null) {
            setShooted(true);
            Vec3d targetVec = new Vec3d(target.posX - this.posX, (target.posY + (double) target.getEyeHeight()) - this.posY, target.posZ - this.posZ).scale(0.25);
            Vec3d courseVec = new Vec3d(target.motionX, target.motionY, target.motionZ);
            ...
            Vec3d montion = courseVec.scale(courseLen / totalLen).add(targetVec.scale(targetLen / totalLen));
    ...
    protected void onImpact(RayTraceResult result) {
        extraAction(result);
        if (getSize() > 5) {
            Explosion explosion = this.world.createExplosion(thrower, this.posX, this.posY, this.posZ, getSize(), false);
        }

...
public class MobEffectDoomedDeathBurning extends Potion {
    private final static String ICON_NAME_0 = Reference.MOD_ID + ":blocks/crimson_flame_layer_0";
    private final static String ICON_NAME_1 = Reference.MOD_ID + ":blocks/crimson_flame_layer_1";

    public MobEffectDoomedDeathBurning(boolean isBadEffectIn, int liquidColorIn) {
        super(isBadEffectIn, liquidColorIn);
        PotionUtil.registerPotion(this, "doomed_death_burning");
        ...
        if (evt.getPotion().equals(getPotion())) {
            ...
            if (minecraftServer != null) {
                for (EntityPlayer entityPlayer : entityLivingBase.world.getMinecraftServer().getPlayerList().getPlayers()) {
                    NetworkRegistryHandler.DoomeDeathBurning.sendClientCustomPacket(entityPlayer, entityLivingBase.getEntityId(), false);
                }
            }
        }
    }
    ...
    public static void onRenderLiving(RenderLivingEvent.Specials.Post evt) {
        ...
        if (NetworkRegistryHandler.DoomeDeathBurning.getEntitiesID().contains(entityLivingBase.getEntityId())) {
    ...
    public static void onRenderSpecificHand(RenderSpecificHandEvent evt) {
        ...
        if (NetworkRegistryHandler.DoomeDeathBurning.getEntitiesID().contains(player.getEntityId())) {
    ...
    public void performEffect(EntityLivingBase entityLivingBaseIn, int amplifier) {
        ...
        if (minecraftServer != null) {
            for (EntityPlayer entityPlayer : entityLivingBaseIn.world.getMinecraftServer().getPlayerList().getPlayers()) {
                NetworkRegistryHandler.DoomeDeathBurning.sendClientCustomPacket(entityPlayer, entityLivingBaseIn.getEntityId(), true);
            }
        }
    }

 ...
 * 观察者客户端的 {@code entity.hasEffect(SCARLET_ROT)} 仍为 {@code false}。为兼顾「可靠」与
 * 「覆盖战斗中的怪」，本渲染器对每个实体用三个条件取并集判定（见 {@link #shouldRenderRot}）：
 * <ol>
 ...
 * 渐隐为 0），放大后呈化开的雾团而非尖锐红点。
 * <p>
 * <b>两类视觉：</b>(1) 普通中毒红雾——脚下水平红雾盘 + 环绕上升的猩红孢子点云；(2) 腐败女神 aeonia
 * 感染源光环——大范围地面腐败浸染盘 + 感染半径内地面各处升腾的孢子柱 +（可选）脚下缓慢旋转的核心孢子环。
 * <p>
 * <b>浓度与淡出：</b>客户端无法可靠取得其他实体效果的等级与剩余时间，故普通红雾统一
 * 基础浓度（{@code amplifier=0}）、不做到期淡出（{@code fade=1}）；出现 / 消失由判定条件控制。
 *
 * <h3>v3（顶点量，近距离视觉零变化）：接入 {@link VisualLod}</h3>
 * <p>
 * 顶点量分两档，<b>腐败女神持有者是全模组单实体第二高</b>（仅次于三重黄金树祝福）：
 * </p>
 * <pre>
 * 普通中毒者：
 *   地面红雾盘（2 层 × 24 段 × 3）        144
 *   上升孢子（16 颗 × 10 段 × 3）         480
 *   ─────────────────────────────────────────
 *   小计                                ~624
 *
 * 腐败女神持有者（在上述基础上叠加）：
 *   浸染盘（3 层 × 32 段 × 3）            288
 *   环境升腾孢子（32 颗 × 10 段 × 3）     960
 *   ─────────────────────────────────────────
 *   合计                               ~1872 顶点 / 实体 / 帧
 * </pre>
 * <p>
 * 且腐败会<b>传播</b>——猩红艾奥尼亚附近死亡的实体有 50% 概率把腐败扩散给 16 格内其他实体，
 * 刷怪塔 / 群体交战时同屏中毒者数量可以相当高，是本模组最容易出现「一大片实体同时带同一种特效」的场景。
 * </p>
 * <p>
 * 现按 {@link VisualLod#detail} 缩放：{@link VisualLod#FULL_DETAIL_RANGE} 格内系数为 1.0，
 * <b>与优化前逐像素一致</b>；40 格外腐败女神持有者降至约 380 顶点（降幅 80%）。
 * </p>
 * <p>
 * <b>感染源孢子必须按步长抽取，不能截断（重要）：</b>环境升腾孢子的分布是
 * {@code ringFrac = (i + 0.5) / 总数}、{@code baseRad = R × sqrt(ringFrac)} 的均匀面积分布——
 * <b>下标 i 越大、半径越靠外</b>。若简单地「只画前 N 颗」，结果是<b>只剩中心一小撮、外圈整个空掉</b>，
 * 大光环会塌成一个小点，视觉完全变形。故改为 {@code for (i = 0; i < 总数; i += step)}，
 * {@code ringFrac} 仍按<b>原始总数</b>计算，保证保留孢子的半径与方位不变、且始终铺满整个感染范围。
 * </p>
 *
 * @author FlameForge
 */
...
public final class ScarletRotMistRenderer {
    ...
    /**
     * 渲染器起始墙钟毫秒（类加载时固定）。
     * <p>动画时间必须用「当前毫秒 - 此起始值」的<b>差值</b>再转 float，而非直接
     * {@code currentTimeMillis()/1000f}：后者数值约 1.7e12，超出 float 有效精度，
     * 会导致每帧算出的时间完全相同、动画彻底静止。</p>
     */
    private static final long START_MILLIS = System.currentTimeMillis();

    // ===== v3 LOD 下限与保留阈值 =====
    /** 孢子柔光圆的最少分段数：4 段仍是个饱满的菱形柔光块 */
    private static final int SPORE_SEGMENTS_MIN = 4;
    /** 地面雾盘 / 浸染盘的最少分段数 */
    private static final int DISC_SEGMENTS_MIN = 8;
    /** 地面红雾盘第二层（更大更淡）的保留阈值 */
    private static final float GROUND_OUTER_KEEP_THRESHOLD = 0.55f;
    /** 感染源浸染盘第三层（最外最淡）的保留阈值 */
    private static final float AEONIA_OUTER_KEEP_THRESHOLD = 0.6f;

    // ===== 配色（0xRRGGBB，取自艾尔登法环猩红主题）=====
    /** 猩红（孢子上升后期的亮红） */
    ...
    public static void onRenderLevel(RenderLevelStageEvent event) {
        ...
        for (LivingEntity entity : candidates) {
            ...
            // ⭐ v3：本实体的细节系数（距离 × 同屏拥挤度）。12 格内恒为 1.0，视觉与优化前一致
            ...
            drawGroundHaze(builder, matrix, rx, ryGround, rz, width, time, 0, 1f, entity.getId(), detail);
            drawRisingSpores(builder, matrix, rx, (float) dy, rz, width, height, time, 0, 1f,
                    entity.getId(), rightX, rightY, rightZ, upX, upY, upZ, detail);
     ...
     * 随时间缓慢呼吸，营造弥漫地表的红雾。
     * <p>v3：分段数按细节系数缩放；低细节时只画内层（第二层是更大更淡的铺底，远处看不出）。</p>
     *
     * @param b         缓冲
     * @param m         变换矩阵
     * @param cx        中心相对相机 X
    ...
    private static void drawGroundHaze(BufferBuilder b, Matrix4f m,
                                       ...
                                       float time, int amplifier, float fade, int seedId, float detail) {
        ...
        int segments = VisualLod.scaleSegments(GROUND_SEGMENTS, DISC_SEGMENTS_MIN, detail);
        int layers = VisualLod.keepLayer(detail, GROUND_OUTER_KEEP_THRESHOLD) ? GROUND_LAYERS : 1;
        ...
        for (int layer = 0; layer < layers; layer++) {
            ...
            drawDisc(b, m, cx, cy, cz, radius, segments, cr, cg, cb, centerAlpha);
        }
    }
     ...
     * 颜色由深红转亮红、伴随细微明灭、随高度螺旋外散。</p>
     * <p>
     * v3：数量与分段数按细节系数缩放。<b>本类孢子的角度是纯随机</b>
     * （{@code ang = rngFloat(s) * TAU}，与下标无关），因此截断尾部安全——
     * 保留孢子的位置与轨迹完全不变，靠近时是「逐渐多冒出几颗」而非重新洗牌。
     * </p>
     *
     * @param detail 细节系数
     */
    private static void drawRisingSpores(BufferBuilder b, Matrix4f m,
        ...
        int baseCount = Math.min(MAX_SPORES, BASE_SPORES + amplifier * SPORES_PER_LEVEL);
        int count = VisualLod.scale(baseCount, detail);
        int segments = VisualLod.scaleSegments(SPORE_SEGMENTS, SPORE_SEGMENTS_MIN, detail);
     ...
     * <ol>
     *     <li><b>大范围地面腐败浸染盘</b>（{@link #AEONIA_GROUND_LAYERS} 层、半径 {@link #AEONIA_FIELD_RADIUS}）；</li>
     *     <li><b>环境升腾孢子</b>（在感染半径内地面各处的稳定位置持续错相升起的孢子柱）；</li>
     *     <li><b>脚下核心孢子环</b>（{@link #AEONIA_CORE_RING_COUNT} 默认 0 关闭）。</li>
     * </ol>
     * </p>
     * <p>
     * <b>v3 削减策略（关键）：</b>浸染盘按细节缩分段、低细节时少画一层；
     * <b>环境孢子必须按步长抽取而非截断</b>——其分布是
     * {@code baseRad = R × sqrt((i + 0.5) / 总数)} 的均匀面积分布，
     * 下标越大半径越靠外，截断前 N 颗会让整个大光环塌成中心一小撮（详见类注释）。
     * </p>
     *
     * @param detail 细节系数
     */
    private static void drawAeoniaInfectionField(BufferBuilder b, Matrix4f m,
        ...
        int discSegments = VisualLod.scaleSegments(AEONIA_GROUND_SEGMENTS, DISC_SEGMENTS_MIN, detail);
        int layers = VisualLod.keepLayer(detail, AEONIA_OUTER_KEEP_THRESHOLD)
                ? AEONIA_GROUND_LAYERS : AEONIA_GROUND_LAYERS - 1;
        ...
        for (int layer = 0; layer < layers; layer++) {
            ...
            float cg = ((col >> 8) & 0xFF) / 255f;
            float cb = (col & 0xFF) / 255f;
            drawDisc(b, m, cx, groundY, cz, radius, discSegments, cr, cg, cb, centerAlpha);
        }

        // —— 2) 环境升腾孢子（地面各处稳定位置错相升起）——
        // ⭐ v3：按步长抽取，绝不能截断——下标决定半径，截断会让光环塌成中心一撮（详见类注释）
        int drawnSpores = VisualLod.scale(AEONIA_FIELD_SPORES, detail);
        int sporeStep = Math.max(1, AEONIA_FIELD_SPORES / drawnSpores);
        int sporeSegments = VisualLod.scaleSegments(SPORE_SEGMENTS, SPORE_SEGMENTS_MIN, detail);

        for (int i = 0; i < AEONIA_FIELD_SPORES; i += sporeStep) {
            // 黄金角螺旋 + sqrt 半径 = 均匀面积分布（稳定不闪跳）。
            // ringFrac 仍按原始总数计算，保证保留孢子的半径与全细节时完全一致
            ...
            emitSoftSpore(b, m, gx, gy, gz, size, cr, cg, cb, alpha,
                    rightX, rightY, rightZ, upX, upY, upZ, sporeSegments);
        }
        ...
        if (AEONIA_CORE_RING_COUNT > 0) {
            ...
            // 均布角度，同样按步长抽取
            int drawnRing = VisualLod.scale(AEONIA_CORE_RING_COUNT, detail);
            int ringStep = Math.max(1, AEONIA_CORE_RING_COUNT / drawnRing);
            for (int i = 0; i < AEONIA_CORE_RING_COUNT; i += ringStep) {
     ...
     * <p>由 {@code segments} 段三角扇拼成一个正对相机的圆——相比中心 + 四角的菱形，
     * 圆形中心不再尖锐、边缘平滑渐隐，放大后像化开的雾团而非尖锐红点。</p>
     *
     * @param segments 分段数。v3 起由调用方按细节系数传入，下限 {@link #SPORE_SEGMENTS_MIN}；
     *                 全细节时即 {@link #SPORE_SEGMENTS}。孢子数量多，是本渲染器的主要顶点杠杆。
     */

...
public final class BadOmenRenderer {
    ...
    private static void drawCurseRing(BufferBuilder b, Matrix4f m,
        ...
        float[] black = unpack(OMEN_BLACK);
        float[] gold = unpack(OMEN_SICK_GOLD);
        ...
        for (int i = 0; i < RING_DASHES; i++) {
            ...
            for (int sIdx = 0; sIdx <= sub; sIdx++) {
                ...
                if (sIdx > 0) {
                    ...
                    float[] col = mix(black, gold, edgeness * 0.7f);

                    b.vertex(m, prevOx, ringY, prevOz).color(col[0], col[1], col[2], alpha).endVertex();
                    b.vertex(m, ox, ringY, oz).color(col[0], col[1], col[2], alpha).endVertex();
                    b.vertex(m, ix, ringY, iz).color(col[0], col[1], col[2], alpha).endVertex();

                    b.vertex(m, prevOx, ringY, prevOz).color(col[0], col[1], col[2], alpha).endVertex();
                    b.vertex(m, ix, ringY, iz).color(col[0], col[1], col[2], alpha).endVertex();
                    b.vertex(m, prevIx, ringY, prevIz).color(col[0], col[1], col[2], alpha).endVertex();
                }
                prevOx = ox;
                prevOz = oz;
                prevIx = ix;
    ...
    private static void drawOoze(BufferBuilder b, Matrix4f m,
        ...
        for (int i = 0; i < OOZE_COUNT; i++) {
            ...
            verticalLine(b, m, px, pz, tailY, headY, OOZE_HALF_WIDTH,
                    black, alpha * 0.4f, mix(black, rust, 0.45f), alpha);
        }
    }
